...
router.put('/walt', [auth_1.AuthenticationMiddleware], async (req, res) => {
    try {
        const { walt } = req.body;
        const IdUser = res.locals.user._id;
        const user = await user_model_1.User.find({ _id: IdUser });
        console.log(user);
        console.log(IdUser);
        const waltAdd = Number(user[0].walt) + Number(walt);
        const update = await user_model_1.User.updateOne({ _id: IdUser }, {
            $set: {
                walt: waltAdd
            }
        });
...
router.put('/buyWalt', [auth_1.AuthenticationMiddleware], async (req, res) => {
    try {
